...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  const shortcutFieldMeta: Record<ShortcutActionId, { label: string; zh: string; ru: string; hintEn: (defaults: string) => string; hintZh: (defaults: string) => string; hintRu: (defaults: string) => string }> = {
    paste: {
      label: 'Paste shortcut',
      zh: '粘贴快捷键', ru: 'Горячая клавиша вставки',
      ...
      hintRu: d => `Вставка из буфера обмена (текст, пути к файлам, изображения). По умолчанию ${d}. Если терминал перехватывает Ctrl+V, используйте Alt+V.`,
    },
    history: {
      label: 'History search shortcut',
      zh: '历史搜索快捷键', ru: 'Горячая клавиша поиска по истории',
      hintEn: d => `Open the prompt-history search. Default: ${d}.`,
      hintZh: d => `打开输入历史搜索。默认 ${d}。`,
      hintRu: d => `Поиск по истории ввода. По умолчанию ${d}.`,
    },
    editor: {
      label: 'External editor shortcut',
      zh: '外部编辑器快捷键', ru: 'Горячая клавиша внешнего редактора',
      hintEn: d => `Edit the draft in $VISUAL/$EDITOR. Default: ${d}.`,
      hintZh: d => `在 $VISUAL/$EDITOR 外部编辑器中编辑草稿。默认 ${d}。`,
      hintRu: d => `Редактирование черновика во внешнем редакторе из $VISUAL/$EDITOR. По умолчанию ${d}.`,
    },
    transcript: {
      label: 'Transcript mode shortcut',
      zh: '转录模式快捷键', ru: 'Горячая клавиша режима транскрипта',
      hintEn: d => `Toggle expanded transcript mode. Default: ${d}.`,
      hintZh: d => `切换展开转录模式。默认 ${d}。`,
      hintRu: d => `Переключение развёрнутого режима транскрипта. По умолчанию ${d}.`,
    },
    trajectory: {
      label: 'Trajectory scene shortcut',
      zh: '轨迹场景快捷键', ru: 'Горячая клавиша сцены траектории',
      hintEn: d => `Open the trajectory scene. Default: ${d}.`,
      hintZh: d => `打开轨迹场景。默认 ${d}。`,
      hintRu: d => `Открыть сцену траектории. По умолчанию ${d}.`,
    },
    dashboard: {
      label: 'Subagent dashboard shortcut',
      zh: '子代理面板快捷键', ru: 'Горячая клавиша панели субагентов',
      hintEn: d => `Open the subagent dashboard. Default: ${d}.`,
      hintZh: d => `打开子代理面板。默认 ${d}。`,
      hintRu: d => `Открыть панель субагентов. По умолчанию ${d}.`,
    },
    contextPanel: {
      label: 'Loaded-context panel shortcut',
      zh: '加载上下文面板快捷键', ru: 'Горячая клавиша панели загруженного контекста',
      hintEn: d => `Toggle the startup loaded-context panel. Default: ${d}.`,
      hintZh: d => `切换启动时的已加载上下文面板。默认 ${d}。`,
      hintRu: d => `Переключить панель загруженного контекста при запуске. По умолчанию ${d}.`,
    },
    showAll: {
      label: 'Show-all shortcut',
      zh: '显示全部消息快捷键', ru: 'Горячая клавиша показа всех сообщений',
      hintEn: d => `Toggle show-all-messages. Default: ${d}.`,
      hintZh: d => `切换显示全部消息。默认 ${d}。`,
      hintRu: d => `Переключить показ всех сообщений. По умолчанию ${d}.`,
    },
    redraw: {
      label: 'Redraw shortcut',
      zh: '终端重绘快捷键', ru: 'Горячая клавиша перерисовки терминала',
      hintEn: d => `Clear and repaint the terminal. Default: ${d}.`,
      hintZh: d => `清空并重绘终端。默认 ${d}。`,
      hintRu: d => `Очистить терминал и перерисовать его. По умолчанию ${d}.`,
    },
    todoFold: {
      label: 'Todo fold shortcut',
      zh: '待办折叠快捷键', ru: 'Горячая клавиша сворачивания списка задач',
      hintEn: d => `Fold/unfold the goal/todo panel. Default: ${d}.`,
      hintZh: d => `折叠/展开目标与待办面板。默认 ${d}。`,
      hintRu: d => `Свернуть/развернуть панель целей и задач. По умолчанию ${d}.`,
    },
    expandEditor: {
      label: 'Fullscreen editor shortcut',
      zh: '全屏草稿编辑快捷键', ru: 'Горячая клавиша полноэкранного редактирования черновика',
      ...
      hintRu: d => `Переключить полноэкранный редактор черновика (Enter — перенос строки, Ctrl+Enter — отправить). По умолчанию ${d}.`,
    },
  }
  const shortcutFields: TuiSettingsField[] = SHORTCUT_ACTIONS.map(action => {
    ...
    return {
      ...
      label: meta.label,
      descriptions: { zh: meta.zh, ru: meta.ru },
      hint: meta.hintEn(defaults),
      hintDescriptions: { zh: meta.hintZh(defaults), ru: meta.hintRu(defaults) },
      group: 'shortcuts',
      kind: 'text',
  ...
  // same registry either way.
  {
    ...
    const unregister = settingsSections.register({
      ...
      title: 'dsh-tui',
      groups: [
        { id: 'status-bar', title: 'Status bar', descriptions: { zh: '底栏设置', ru: 'Настройки нижней строки' } },
        { id: 'shortcuts', title: 'Shortcuts', descriptions: { zh: '快捷键', ru: 'Горячие клавиши' } },
        { id: 'session', title: 'Session', descriptions: { zh: '会话', ru: 'Сессия' } },
      ],
      fields: [
        {
          path: ['lang'],
          label: 'Language',
          descriptions: { zh: '界面语言', ru: 'Язык интерфейса' },
          hint: 'UI language for the whole interface — applies immediately and is saved.',
          hintDescriptions: { zh: '整个界面的显示语言——立即生效并保存。', ru: 'Язык отображения всего интерфейса — применяется сразу и сохраняется.' },
          kind: 'select',
          options: [
        ...
        },
        {
          ...
          descriptions: { zh: '全屏模式', ru: 'Полноэкранный режим' },
          ...
          hintDescriptions: { zh: '开启：接管整个终端（同 vim/less），应用内鼠标；关闭：终端原生滚动选择；整屏页两种模式都有鼠标。重启生效。', ru: 'Включено: перехват всего терминала (как vim/less), мышь обрабатывает приложение; выключено: родная прокрутка и выделение текста терминалом. Полноэкранные страницы используют мышь приложения в обоих режимах. Требуется перезапуск.' },
          kind: 'boolean',
          format(value: unknown): string {
        ...
        },
        {
          ...
          descriptions: { zh: terminalImagesDisabledByEnv ? '图片预览（环境强制关闭）' : '终端图片预览', ru: terminalImagesDisabledByEnv ? 'Предпросмотр изображений (принудительно отключён окружением)' : 'Предпросмотр изображений в терминале' },
          ...
          hintDescriptions: {
            ...
            ru: terminalImagesDisabledByEnv
              ? 'Флажок сохраняет настройку предпросмотра; чтобы изображения отображались, уберите DSH_TUI_DISABLE_TERMINAL_IMAGES и перезапустите приложение.'
              : 'Предпросмотр изображений в поддерживаемых терминалах. После изменения применяется через /restart; на отправку изображений модели не влияет.',
          },
          kind: 'boolean',
          format(value: unknown): string {
        ...
        },
        {
          path: ['diffLayout'],
          label: 'Diff layout',
          descriptions: { zh: 'diff 布局', ru: 'Макет diff' },
          hint: 'Edit/Write tool cards: auto picks by terminal width, or force one layout.',
          hintDescriptions: { zh: 'Edit/Write 工具卡的 diff 呈现：auto 按终端宽度选择，或强制一种布局。', ru: 'Отображение diff в карточках инструментов Edit/Write: auto выбирает вариант по ширине терминала, либо макет задаётся принудительно.' },
          kind: 'select',
          options: [
            { value: 'auto', label: 'Auto (by width)', descriptions: { zh: '自动（按宽度）', ru: 'Авто (по ширине)' } },
            { value: 'split', label: 'Side-by-side', descriptions: { zh: '双栏对照', ru: 'Две колонки' } },
            { value: 'unified', label: 'Unified', descriptions: { zh: '统一式', ru: 'Унифицированный' } },
          ],
        },
        {
          path: ['thinkingFold'],
          label: 'Thinking display',
          descriptions: { zh: '思考块展示', ru: 'Отображение блоков размышлений' },
          ...
          hintDescriptions: { zh: '预览模式显示 2-3 行动态思考；展开模式保持至轮末。点击流式思考块可在预览与全文间切换。', ru: 'В режиме предпросмотра показывается 2–3 строки текущих размышлений; в раскрытом режиме блок остаётся раскрытым до конца хода. Клик по потоковому блоку размышлений переключает предпросмотр и полный текст.' },
          kind: 'select',
          options: [
            { value: 'preview', label: 'Preview (2-3 lines)', descriptions: { zh: '预览（2-3 行）', ru: 'Предпросмотр (2–3 строки)' } },
            { value: 'full', label: 'Full until turn end', descriptions: { zh: '展开至轮末', ru: 'Раскрыто до конца хода' } },
          ],
        },
        {
          path: ['toolBackground'],
          label: 'Tool background',
          descriptions: { zh: '工具卡背景', ru: 'Фон карточек инструментов' },
          hint: 'Choose whether tool-call cards add no, subtle, or strong background emphasis.',
          hintDescriptions: { zh: '选择工具调用卡片不添加、轻微或明显的背景强调。', ru: 'Выберите фоновый акцент для карточек вызовов инструментов: отсутствие, лёгкий или заметный.' },
          kind: 'select',
          options: [
            { value: 'none', label: 'None', descriptions: { zh: '无', ru: 'Нет' } },
            { value: 'subtle', label: 'Subtle', descriptions: { zh: '轻微', ru: 'Лёгкий' } },
            { value: 'strong', label: 'Strong', descriptions: { zh: '明显', ru: 'Заметный' } },
          ],
        },
        {
          path: ['scrollGutter'],
          label: 'Transcript gutter',
          descriptions: { zh: '转录边栏', ru: 'Боковая панель транскрипта' },
          hint: 'Right gutter of the fullscreen transcript: per-turn timeline ticks, a proportional scrollbar, or nothing.',
          hintDescriptions: { zh: '全屏转录区右侧边栏：按轮次的时间线节点、比例滚动条，或留空。', ru: 'Правая боковая панель полноэкранного транскрипта: узлы временной шкалы по ходам, пропорциональная полоса прокрутки или пусто.' },
          kind: 'select',
          options: [
            { value: 'timeline', label: 'Turn timeline', descriptions: { zh: '轮次时间线', ru: 'Временная шкала ходов' } },
            { value: 'scrollbar', label: 'Scrollbar', descriptions: { zh: '滚动条', ru: 'Полоса прокрутки' } },
            { value: 'hidden', label: 'Hidden', descriptions: { zh: '隐藏', ru: 'Скрыть' } },
          ],
        },
        {
          path: ['pageMargin'],
          label: 'Page margin',
          descriptions: { zh: '页边距', ru: 'Поля' },
          ...
          hintDescriptions: { zh: '让整个界面相对终端四边内缩。←/→ 循环预设（none / slim / normal / roomy）；Enter 输入自定义 `NxM`：左右各 N 列、上下各 M 行（如 3x1，上限 8x4；只填 N 则上下保持 1 行）。清空恢复默认 normal。立即生效。', ru: 'Внутренний отступ всего интерфейса от краёв терминала. ←/→ перебирают пресеты (none / slim / normal / roomy); Enter — свой формат `NxM`: по N колонок слева и справа, по M строк сверху и снизу (например 3x1, максимум 8x4; если указать только N, сверху и снизу останется 1 строка). Очистка возвращает значение по умолчанию normal. Применяется сразу.' },
          kind: 'text',
          placeholder: 'normal',
          options: [
            { value: 'none', label: 'None', descriptions: { zh: '无', ru: 'Нет' } },
            { value: 'slim', label: 'Slim', descriptions: { zh: '窄', ru: 'Узкие' } },
            { value: 'normal', label: 'Normal', descriptions: { zh: '常规', ru: 'Обычные' } },
            { value: 'roomy', label: 'Roomy', descriptions: { zh: '宽', ru: 'Просторные' } },
          ],
          format(value: unknown): string {
        ...
        },
        {
          ...
          descriptions: { zh: '折叠终端命令', ru: 'Сворачивать команды терминала' },
          ...
          hintDescriptions: { zh: '终端卡（Bash/PowerShell）：多行命令头部折叠为首行 + 计数；Ctrl+O 或点击卡片展开。', ru: 'Карточки терминала (Bash/PowerShell): заголовок многострочной команды сворачивается в первую строку и счётчик; Ctrl+O или клик по карточке раскрывает.' },
          kind: 'boolean',
          format(value: unknown): string {
        ...
        },
        {
          ...
          descriptions: { zh: '会话名标签', ru: 'Метка имени сессии' },
          hint: 'Show the session name on the prompt top border, right corner. Off by default.',
          hintDescriptions: { zh: '在输入框顶边框右上角显示会话名。默认关闭。', ru: 'Показывать имя сессии в правом верхнем углу рамки поля ввода. По умолчанию выключено.' },
          kind: 'boolean',
        },
        {
          path: ['expandEditor'],
          label: 'Fullscreen draft editor',
          descriptions: { zh: '全屏草稿编辑', ru: 'Полноэкранное редактирование черновика' },
          ...
          hintDescriptions: { zh: '开启：输入行尾 ⛶ 按钮与全屏编辑快捷键（默认 Ctrl+Shift+E）把草稿展开成整屏编辑器（Enter 换行、Ctrl+Enter 发送）。关闭：两个入口都不显示。默认开启。', ru: 'Включено: кнопка ⛶ в конце строки ввода и горячая клавиша полноэкранного редактирования (по умолчанию Ctrl+Shift+E) разворачивают черновик в редактор на весь экран (Enter — перенос строки, Ctrl+Enter — отправить). Выключено: оба входа скрыты. По умолчанию включено.' },
          kind: 'boolean',
          format(value: unknown): string {
        ...
        },
        {
          ...
          descriptions: { zh: '流式平滑输出', ru: 'Плавный потоковый вывод' },
          ...
          hintDescriptions: { zh: '把实时回复、展开的思考与工具卡正文按 ~30fps 匀速揭示，不再随供应商突发一跳一跳；一次性到达的非流式回复也会平滑打出。回放/历史内容始终完整直出。默认开启。', ru: 'Равномерно, ~30 к/с, показывает ответы в реальном времени, раскрытые размышления и текст карточек инструментов — без рывков от всплесков провайдера; непотоковые ответы, пришедшие целиком, тоже печатаются плавно. Воспроизведение и история всегда выводятся сразу полностью. По умолчанию включено.' },
          kind: 'boolean',
          format(value: unknown): string {
            // Unset in settings.yaml: the effective default is on.
        ...
        },
        {
          ...
          descriptions: { zh: '打开会话时自动总结', ru: 'Автоматическое резюме при открытии сессии' },
          ...
          hintDescriptions: { zh: '开启：打开/恢复会话时自动把最近活动总结成一行灰字显示在会话底部（可悬停/点击查看或应用建议标题）；关闭：手动使用 /recap。', ru: 'Включено: при открытии или восстановлении сессии последняя активность автоматически сводится в одну серую строку внизу сессии (наведение или клик — просмотр и применение предложенного заголовка); выключено: запускайте /recap вручную.' },
          kind: 'boolean',
          format(value: unknown): string {
        ...
        },
        {
          ...
          descriptions: { zh: '默认推理强度', ru: 'Интенсивность рассуждений по умолчанию' },
          ...
          hintDescriptions: { zh: '新会话起始的推理强度档位；模型提供该档位时，当前会话的下一请求也会应用（模型不提供的档位会静默回落到模型默认）。自动 = 依次跟随 cordis.yml 的 effort 配置、持久化的 /effort 选择、模型默认档。', ru: 'Начальный уровень интенсивности рассуждений для новых сессий; если модель поддерживает его, применяется и к следующему запросу текущей сессии (неподдерживаемые уровни молча откатываются к значению модели по умолчанию). Авто = по очереди: настройка effort в cordis.yml, сохранённый выбор /effort, уровень модели по умолчанию.' },
          kind: 'select',
          options: [
            { value: 'auto', label: 'Auto (model default)', descriptions: { zh: '自动（模型默认）', ru: 'Авто (по умолчанию в модели)' } },
            { value: 'off', label: 'Off', descriptions: { zh: '关闭', ru: 'Выкл.' } },
            { value: 'low', label: 'Low', descriptions: { zh: '低', ru: 'Низкая' } },
            { value: 'high', label: 'High', descriptions: { zh: '高', ru: 'Высокая' } },
            { value: 'max', label: 'Max', descriptions: { zh: '最高', ru: 'Максимальная' } },
          ],
          format(value: unknown): string {
        ...
        ...shortcutFields,
        {
          ...
          descriptions: { zh: '紧凑状态栏', ru: 'Компактный статус-бар' },
          hint: 'Prefer the compact status presentation when terminal space allows.',
          hintDescriptions: { zh: '终端空间允许时优先使用紧凑状态栏布局。', ru: 'Когда место в терминале позволяет, использовать компактный макет статус-бара.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          label: 'Show model',
          descriptions: { zh: '显示模型', ru: 'Показывать модель' },
          hint: 'Show the live model id in the status bar.',
          hintDescriptions: { zh: '在状态栏显示当前模型标识。', ru: 'Показывать идентификатор текущей модели в статус-баре.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          label: 'Show thinking',
          descriptions: { zh: '显示思考', ru: 'Показывать размышления' },
          hint: 'Show the live reasoning effort or thinking mode.',
          hintDescriptions: { zh: '显示当前推理强度或思考模式。', ru: 'Показывать текущую интенсивность рассуждений или режим размышлений.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示工作目录', ru: 'Показывать рабочую директорию' },
          hint: 'Show the session working directory.',
          hintDescriptions: { zh: '显示当前会话的工作目录。', ru: 'Показывать рабочую директорию текущей сессии.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示上下文用量', ru: 'Показывать использование контекста' },
          hint: 'Show current context-window consumption.',
          hintDescriptions: { zh: '显示当前上下文窗口占用情况。', ru: 'Показывать заполнение текущего окна контекста.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          label: 'Show cache',
          descriptions: { zh: '显示缓存', ru: 'Показывать кэш' },
          hint: 'Show prompt-cache hit information.',
          hintDescriptions: { zh: '显示提示词缓存命中信息。', ru: 'Показывать статистику попаданий в кэш промптов.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示 Token 总量', ru: 'Показывать общее количество Token' },
          hint: 'Show running input and output token totals.',
          hintDescriptions: { zh: '显示累计输入与输出 Token。', ru: 'Показывать суммарные входные и выходные Token.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示本会话花费估算', ru: 'Показывать оценку стоимости сессии' },
          ...
          hintDescriptions: { zh: '在 Token 总量旁显示本会话花费估算（≈¥）。仅在使用 DeepSeek 官方 API key 且模型有已知单价时显示；按官方每百万 token 单价（高峰/空闲时段）估算，非账单。', ru: 'Рядом с общим количеством Token показывать оценку стоимости текущей сессии (≈¥). Отображается только при официальном API-ключе DeepSeek и известной цене модели; расчёт по официальным ценам за миллион token (часы пик и спад), это не биллинг.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示输出速度', ru: 'Показывать скорость вывода' },
          hint: 'Show live and recent tokens-per-second metrics.',
          hintDescriptions: { zh: '显示实时及近期每秒 Token 指标。', ru: 'Показывать текущие и недавние метрики Token в секунду.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示 Git 分支', ru: 'Показывать ветку Git' },
          hint: 'Show the current git branch when available.',
          hintDescriptions: { zh: '可用时显示当前 Git 分支。', ru: 'Показывать текущую ветку Git, когда это доступно.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示会话标题', ru: 'Показывать заголовок сессии' },
          hint: 'Show the current session title.',
          hintDescriptions: { zh: '显示当前会话标题。', ru: 'Показывать заголовок текущей сессии.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示会话 ID', ru: 'Показывать ID сессии' },
          hint: 'Show the short session id (# + first 8 chars) — it matches the session log filename for --resume.',
          hintDescriptions: { zh: '显示短会话 ID（# + 前 8 位）——与日志文件名对应，方便 --resume 定位。', ru: 'Показывать короткий ID сессии (# и первые 8 символов) — соответствует имени файла журнала, удобно для поиска через --resume.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示 Goal 状态', ru: 'Показывать состояние Goal' },
          hint: 'Show a compact goal chip (phase glyph + rounds) in the status footer while a goal exists.',
          hintDescriptions: { zh: '存在 Goal 时，在底部状态栏显示紧凑的 Goal 状态（阶段符号与轮次）。', ru: 'При наличии Goal показывать в нижнем статус-баре компактное состояние Goal (символ фазы и раунды).' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示会话模式', ru: 'Показывать режим сессии' },
          hint: 'Show the active non-default session mode.',
          hintDescriptions: { zh: '显示当前启用的非默认会话模式。', ru: 'Показывать текущие включённые нестандартные режимы сессии.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示上下文进度条', ru: 'Показывать прогресс-бар контекста' },
          hint: 'Show the segmented context progress bar on its own footer row.',
          hintDescriptions: { zh: '在底部单独一行显示分段上下文进度条。', ru: 'Показывать внизу отдельной строкой сегментированный прогресс-бар контекста.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示活动摘要', ru: 'Показывать сводку активности' },
          hint: 'Show the idle working-activity summary.',
          hintDescriptions: { zh: '显示空闲时的工作活动摘要。', ru: 'Показывать сводку рабочей активности во время простоя.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示轨迹条', ru: 'Показывать шкалу траектории' },
          hint: 'Show the animated mini trajectory strip at the footer edge.',
          hintDescriptions: { zh: '在状态栏边缘显示动态迷你轨迹条。', ru: 'Показывать на краю статус-бара динамическую мини-шкалу траектории.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          ...
          descriptions: { zh: '显示快捷键提示', ru: 'Показывать подсказку о горячих клавишах' },
          hint: 'Control only the idle `? for shortcuts` reminder; pressing ? and the Esc shortcut hints are unaffected.',
          hintDescriptions: { zh: '仅控制空闲时的 `? for shortcuts` 提示；按 ? 打开快捷键以及 Esc 快捷提示均不受影响。', ru: 'Управляет только подсказкой `? for shortcuts` в простое; открытие списка горячих клавиш по ? и быстрые подсказки Esc не затрагиваются.' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
          path: ['whale'],
          label: 'Whale art',
          descriptions: { zh: '鲸鱼娘', ru: 'Девочка-кит' },
          hint: 'Show the pixel whale in the header splash.',
          hintDescriptions: { zh: '开屏头部显示像素鲸鱼娘。', ru: 'Показывать пиксельную девочку-кита в приветственном заголовке.' },
          kind: 'boolean',
        },
        {
          path: ['whaleIdle'],
          label: 'Welcome whale idle',
          descriptions: { zh: '鲸鱼娘闲置动画（欢迎期）', ru: 'Анимация девочки-кита в простое (период приветствия)' },
          ...
          hintDescriptions: { zh: '欢迎期闲置行为：开屏后鲸鱼娘摆鱼鳍、偶尔拍尾巴，空闲会睡着冒 Z；点击唤醒睡着的鲸鱼娘并冒爱心。开始第一个任务后定格为静态标准帧。', ru: 'Поведение в период приветствия: после запуска девочка-кит шевелит плавниками, иногда бьёт хвостом, в простое засыпает с буквами Z; клик будит уснувшую девочку-кита, и появляются сердечки. После начала первой задачи замирает в стандартной статичной позе.' },
          kind: 'boolean',
        },
        {
          path: ['minimal'],
          label: 'Minimal mode',
          descriptions: { zh: '极简模式', ru: 'Минималистичный режим' },
          ...
          hintDescriptions: { zh: '隐藏开屏头部、emoji 状态符与装饰性配色；代码高亮与工具配色保留，底栏只留模型与目录。', ru: 'Скрывает приветственный заголовок, emoji-символы состояния и декоративные цвета; подсветка кода и цвета инструментов остаются, в нижней строке только модель и директория.' },
          kind: 'boolean',
        },
      ],
    })
